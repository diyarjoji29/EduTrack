...
$semesters = range(1, 8); // Check for semesters 1 to 8
...
foreach ($semesters as $sem) {
    ...
    if ($table_exists) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $all_results[$sem] = $row;
            $subject_codes = array_keys($all_results[$sem]);
            ...
            if (!empty($subject_codes)) {
                ...
                $stmt_sub = $conn->prepare($query);
                $stmt_sub->bind_param(str_repeat('s', count($subject_codes)), ...$subject_codes);
                $stmt_sub->execute();
...
<body>
    ...
    <div class="main-content">
        ...
        <?php if (empty($all_results)): ?>
            <p>No results found for registration number: <?php echo strtoupper($ward_reg_no); ?>.</p>
        <?php else: ?>
            <?php foreach ($all_results as $semester => $grades): ?>
